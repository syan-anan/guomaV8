# -*- coding: utf-8 -*-
"""CRNN3 推理封装：cnn/crnn3_ocr.onnx -> (text, conf)，接口同 crnn_ocr。"""
import pathlib
import numpy as np
import cv2
...
def _session():
    ...
    try:
        ...
        p = pathlib.Path(__file__).resolve().parent.parent / "cnn" / "crnn3_ocr.onnx"
        if not p.exists():
            return None
