...
def solve_ocr(img_src, type_code=1001):
    ...
    for v in versions:
        for _ in range(OCR_RETRY):
            raw = ocr.classification(cv2.imencode(".png", v)[1].tobytes())
            if raw:
    ...
    best = collections.Counter(long_ones).most_common(1)[0][0]
    return {"text": best, "raw": best, "confidence": min(1.0, len(long_ones) / len(candidates))}
""")
f.close()
print("done")
